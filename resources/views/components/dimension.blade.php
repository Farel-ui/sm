    ...
    <div class="flex flex-wrap justify-center gap-6">
      @foreach ($dimensions as $dimension)
        <div class="bg-blue-600 text-white rounded-2xl w-[300px] p-6 shadow-lg flex flex-col items-center text-center">
          <img src="{{ asset('images/dimension/' . $dimension->image) }}"
               alt="{{ $dimension->name }}"
               class="w-[111px] h-[73px] mb-4">

          <h3 class="text-xl font-semibold mb-2">{{ $dimension->name }}</h3>
          <p class="text-sm mb-4">{!! $dimension->description !!}</p>

          <!-- Tombol play video -->
          <button onclick="showVideo('{{ asset('video/' . $dimension->video) }}')"
                  class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-yellow-400 hover:text-white transition text-sm font-bold">
            Selengkapnya
          </button>
        </div>
      @endforeach
  ...
  <div id="videoModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center hidden z-50">
    <div id="videoContainer" class="relative w-[90%] max-w-3xl">
      ...
      <button onclick="closeVideo()"
              class="absolute top-2 right-2 bg-white text-black px-3 py-1 rounded-full font-bold">
        X
      </button>
    </div>
  </div>
