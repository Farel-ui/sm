...
<body class="gradient-bg min-h-screen">
    ...
    <main class="pb-12 ml-64">
        ...
        <div class="flex items-center space-x-2">
                <i class="fas fa-star text-3xl mr-2"></i>
            <span class="font-semibold text-xl">Dashboard Penilaian Smart City</span>
        </div>
        <!-- Kanan -->
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-xl mt-6 card-hover fade-in border border-blue-100">
                ...
                <div class="px-6 py-5 table-header">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                <i class="fas fa-search absolute rounded-xl left-7 top-1/2 transform -translate-y-1/2 text-blue-600"></i>
                                <input type="text" placeholder="Cari penilaian..." id="searchInput"
                                    class="ml-4 pl-10 pr-8 py-2.5 text-blue-600  text-sm border border-blue-600 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-blue-500 transition-all" />
                            </div>
                        ...
                        <tbody class="bg-white divide-y divide-blue-500">
                            ...
                            @foreach ($penilaians as $index => $pn)
                                <tr class="table-row hover:bg-blue-50 transition-all duration-200 searchable-row">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap searchable-year">
                                        <div class="text-lg text-blue-500">
                                            {{ $pn->year }}
                                        </div>
                                    </td>
                                    <td class="px-29 py-4 whitespace-nowrap text-center">
                                        <div class="flex justify-center space-x-2 ml-120">
                                            <button class="action-btn bg-blue-500 text-white hover:text-yellow-500"
                                                    title="View Details">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                            <a href="{{ route('admin.penilaian.edit', $pn->id ?? '#') }}"
                                            ...
                                            <!-- Form hapus (hidden) -->
                                            <form id="delete-form-{{ $pn->id }}"
                                                action="{{ route('admin.penilaian.destroy', $pn->id) }}"
                                                method="POST"
                                                style="display:none;">
                                                @csrf
                                                @method('DELETE')
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            searchInput.addEventListener('input', function(e) {
                ...
                searchableRows.forEach(row => {
                    const score = row.querySelector('.searchable-title').textContent.toLowerCase();
                    const year = row.querySelector('.searchable-year').textContent.toLowerCase();
                    if (score.includes(searchTerm) || year.includes(searchTerm)) {
                        row.style.display = '';
                    } else {
