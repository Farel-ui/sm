...
<body class="bg-blue-50 min-h-screen font-sans">
  ...
  <!-- Form -->
  <form action="{{ route('masterplan.update', $masterplan->id) }}" method="POST" enctype="multipart/form-data" class="flex-1 flex flex-col">
    @csrf
    @method('PUT')
        ...
        <div>
          ...
          <div class="flex items-center space-x-6 mt-2">
            <label class="inline-flex items-center">
              <input type="radio" name="status" value="publish" {{ old('status', $masterplan->status) == 'publish' ? 'checked' : '' }} class="form-radio text-blue-600">
              <span class="ml-2">PUBLISH</span>
            </label>
            <label class="inline-flex items-center">
              <input type="radio" name="status" value="draft" {{ old('status', $masterplan->status) == 'draft' ? 'checked' : '' }} class="form-radio text-blue-600">
